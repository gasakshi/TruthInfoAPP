...
def scrape_text_from_link(url):
    ...
    if content :
        ...
        if is_quran_page_link(url):
            print("content: ",content_to_store)
...
def is_quran_page_link(link):
    return re.match(r'^https://equranlibrary\.com/quran/\d+$', link) or re.match(r'^https://equranlibrary\.com/hadith/bukhari/\d+/\d+$', link) 
        ...
        if content:
            ...
            for link in links:
                ...
                if not is_unwanted_link(link) and is_valid_url(full_link, base_domain) and full_link not in visited:
                    # print(f"Queueing: {full_link}")  # Debug outpu
                    stack.append((full_link, depth + 1))

if __name__ == "__main__":
    driver = webdriver.Chrome()
    start_url = 'https://equranlibrary.com/hadithlist/bukhari/1358'
