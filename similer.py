...
async def query_index_async(index_name, query_vector_list):
    ...
    results = await asyncio.to_thread(index.query, vector=query_vector_list, top_k=100, include_metadata=True)
    return results
    ...
    if not all_results:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for idx, (doc_id, metadata) in enumerate(zip(document_ids, metadata_list)):
                ...
                'max_tokens': 250,
                'messages': [
                    {"role": "system", "content": "You are a helpful assistant. Provide answers to the question based solely on the provided document content without altering or manipulating the information of the document. Respond with 'Don't know' for insufficient information or 'Document does not contain information' for unrelated content."},
...
def is_response_relevant(response_text, keywords):
    # print("enter in is relevant:",response_text)
